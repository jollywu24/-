...
- 已在红眼时降至 `80` 或以下：退出红眼。
- 红眼基础倍率：`×1.5`。
- `140–159`：进入 `鬼压桌`，仍属于红眼状态，红眼基础倍率升级为 `×2`，右侧入口显示为 `鬼压赌注`。
- 达到 `160`：爆牌失败。
...
`红眼赌注` 本身只绑定下一次摊牌，不跨轮常驻。但特殊赌注写明的跨轮后果会登记在运行时状态中，例如：

- `借鬼钱`：过关后下一轮上头 `+25`。
- `偷过线`：若因此过关，下一轮上头 `+30`。
- `欠命`：若确实救下爆牌并过关，下一轮上头至少为 `90`。

这些跨轮后果必须在清除 `activeRedEyeBet` 前先结算并登记，之后由下一轮流程消费。
...
- 将红眼状态机与 DOM 表现解耦。
- 继续扩展浏览器流程测试，覆盖更多红眼赌注、跨轮后果、跨轮重置和爆牌优先级。
