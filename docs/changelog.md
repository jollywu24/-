# 更新记录

## 2026-06-17

### 上线目标

- 新增 `docs/launch_goal.md`，将“上线 MVP / Steam 1000 愿望单”拆成公开试玩标准、P0 冒烟项、Steam 页面材料、截图/短视频需求和不上线范围。
- `PROJECT_CONTEXT.md` 增加上线目标改动导航和进一步阅读入口。
- GitHub Pages 发布流程改为上传 `dist-pages/`，其中包含 `web/`、`art/` 和 `public/`，避免线上缺少本地美术资源。
- 更新 `ARCHITECTURE.md`、`TASKS.md` 和 `.github_pages.md`，同步当前部署方式和后续资源可访问性检查任务。
- 增加 `web/tests/deploy-assets.test.mjs`，用 Node 测试保护 Pages artifact 目录策略和关键运行时图片路径。
- 在 `docs/launch_goal.md` 中补充上线前手动 QA 清单，覆盖启动布局、核心操作、红眼/失败/通关、商店和线上资源。
- 改进 `web/tests/browser-flow.test.mjs` 的浏览器启动方式，自动查找 Chrome/Edge，并用固定 DevTools 端口提升测试可诊断性。
- 修正红眼赌注在通关结算、爆牌失败或庄家通吃分支中未及时清除的问题，确保红眼赌注只绑定下一次摊牌。
- 修正商店进入下一轮时引用不存在的 `hand` 变量导致流程卡死的问题，并用浏览器流程测试覆盖。
- 扩展 `docs/launch_goal.md` 的 Steam 愿望单转化材料，补充商店页文案草案、标签建议、截图/短视频脚本、胶囊图需求和反馈收集入口。
- 明确截图、浏览器 QA 和资源处理优先使用现成工具，不在项目中重复维护通用截图脚本。
- 将上线目标进一步拆成 Gate A 公开试玩、Gate B Steam Coming Soon、Gate C 1000 愿望单验证三道发布闸门，并记录当前证据与待确认项。
- 补齐 Gate A 的版本定位证据：运行时保留 `window.__BUILD_SHA__ || 'dev'` 占位，Debug 快照暴露 `buildSha`，部署测试保护 Pages SHA 注入步骤。
- 记录 `2026-06-22` 线上 Pages HTTP 冒烟结果：`/web/` 和关键图片资源均返回 `200 OK`，同时保留最新 `main` 推送后复查任务。
- 推送最新 `main` 后确认 Pages workflow 对 `9151a0c` 部署成功，并复查线上入口、关键图片资源和 `app.js` build SHA 注入结果。
- 浏览器流程测试新增桌面与手机横屏布局断言，覆盖固定 `1600 x 900` 画布无滚动、手牌不压右侧操作区和右侧操作区不越界。
- 增加公开试玩反馈入口：GitHub Issue Form 收集版本信息、玩法理解、红眼使用、重开欲望和愿望单意愿；新增 `web/feedback.html` 作为独立反馈说明页，不改动主游戏界面。
- 推送 `dd50697` 后复查线上 `/web/feedback.html` 返回 `200 OK`，确认公开反馈说明页已经进入 GitHub Pages。
- 新增 `Web MVP Tests` GitHub Actions 工作流，使用 Node `22` 在主干、PR 和手动触发时运行语法检查、Node 测试、浏览器流程测试和空白检查；部署资源测试同步保护该测试工作流。
- 复查 `9074664` 的 `Web MVP Tests` 和 Pages 部署均成功，线上 `/web/`、反馈页、关键图片资源和 build SHA 均通过冒烟检查。
- README 补充线上试玩入口、反馈说明页和 GitHub Issue Form，方便直接发给公开测试玩家；部署资源测试同步保护这些入口链接。
- 修复 Windows headless 浏览器 profile 清理偶发 `EBUSY` 导致 `web/tests/browser-flow.test.mjs` 误失败的问题；本机重跑浏览器流程测试通过，并将当前 Gate A 证据同步到上线目标文档。
- 复查 `2009dc8` 的 `Web MVP Tests` 和 Pages 部署均成功，线上试玩入口、反馈页、关键图片资源、`web/app.js` 和 build SHA 均通过 HTTP 冒烟检查。
- Pages 部署工作流新增部署后线上 URL 冒烟检查，自动验证试玩入口、反馈页、关键图片资源、`web/app.js` 和构建 SHA；部署资源测试同步保护该步骤。
- 放宽 `web/tests/browser-flow.test.mjs` 的 Chrome DevTools 冷启动等待时间，避免 GitHub runner 首次启动浏览器偏慢时误报失败。
- 产出首批 Steam 页面真实 gameplay 截图到 `art/steam/screenshots/`，覆盖主界面、摊牌结算、红眼赌注、暗涌翻牌、通关收益、商店和爆牌失败；截图生成使用现成 Chrome / DevTools 能力，没有新增仓库内截图脚本。
- 将新增赌鬼牌图规范化为 5 张 `512 x 768` JPG 头像，并接入 `web/asset-map.js`、赌鬼卡渲染和部署资源测试。
- 上头阶段显示收敛为 `普通 -> 红眼 -> 鬼压桌 -> 爆牌`，移除 `热手` 作为阶段文案；`鬼压桌` 状态下红倍率升级为 `×2`，右侧入口显示 `鬼压赌注`，过关时额外结算 `(上头值 - 139) × 2` 的鬼压桌过关奖励。
- 抽牌进入手牌的飞行卡背改为复用最新 `cards/deck_back.png`，与右侧牌堆卡背保持一致，加载失败时仍回退到 CSS 牌背。
- 修正 `烂命保险` 只在摊牌前已处于红眼时才可触发的问题；现在本手结算中进入红眼并撞到爆牌线，也会被保险拦截并设为 `120`。
- 将 `art/shop.png` 替换为不含烘焙 UI 的 `1600 x 900` 黑暗禁忌商店环境图；商店弹窗、操作栏与货架改为透明交互层，移除重复的旧 CSS 货架和立柱，同时保留 DOM 商品、价格与按钮。
- 修正商店环境图被主赌桌底图遮住的层级问题，商店阶段背景现在固定显示在赌桌底图之上、商品交互层之下。
- UI 交付流程新增强制截图验收：进入对应真实状态，在 `1600 x 900` 视口等待关键图片解码与动画稳定后截图检查，再允许交付。

## 2026-06-06

### 当前状态

- 上头值最大值调整为 `160`，达到 `160` 时爆牌。
- 红眼改为 `100` 进入、`80` 或以下退出，并在红眼状态下提供本手倍率 `×1.5`。
- 普通关、精英关、Boss 关过关减压分别调整为 `25`、`35`、`50`。
- 商店加入可花费赌资购买的降压道具。
- 商店赌鬼池替换为 5 张已完整接入结算的红眼流派赌鬼牌：血丝眼镜、红眼借据、小牌壮胆、烂命保险、戒断反弹。
- 移除商店中未接入实际结算的旧赌鬼牌。
- 红眼改为全局视觉状态，加入普通、预热、红眼、临界、下注和爆牌状态 class。
- 补充进入/退出红眼、临界、爆牌短动画，并实现暗涌牌从右上牌堆飞出、翻面和上头值飞入表盘的结算动画。
- 红眼赌注生效时，上头值不再叠加牌型上头，只取暗涌牌点数；`欠命` 救下本应爆牌的手牌时，上头设为 `120`，并登记下一轮上头至少为 `90`。
- 赌鬼改变红倍率时，会在对应赌鬼牌下方弹出带暗红底图的倍率飘字。
- 初始化时不再显示默认赌鬼卡。
- 顶部赌鬼区保留位置，但初始计数为 `0 / 5`，只在获得赌鬼后显示具体卡牌。
- 红眼赌注弹窗按效果图方向还原为暗红边框、顶部红眼装饰、三张赌注卡和底部选择提示。
- 浏览器流程测试增加 `?debug=1` 测试接口，覆盖红眼下注、通关结算、商店购买、进入下一轮、爆牌失败、庄家通吃和失败后重开分支。

## 2026-06-05

### 文档整理

- 对齐当前主界面规范：五个常驻区域保持不变，红眼选择、通关结算和失败结果只作为事件触发弹窗出现。
- 补充当前红眼赌注池、暗涌牌、通关收益结算和失败弹窗规则。
- 更新 MVP 状态：通关收益弹窗、失败弹窗和商店基础流程已接入；出售、牌型升级、完整开包和 Boss 展示仍未完成。
- 将稳定规则集中到 `ui_spec.md` 和 `component_rules.md`，`changelog.md` 只保留变更摘要，避免重复维护同一套规则。

## 2026-06-03

### 当前状态

- 手牌在初始化发牌、换牌补牌、摊牌补牌和下一轮重新发牌后会自动按点数和花色排序。
- 摊牌时，已选牌按当前手牌显示顺序进入中央出牌区。
- 结算时只对牌型有效牌触发单牌筹码表现，高牌等牌型不会让无效牌重复计分。
- 出牌区筹码飘字改为单个蓝底数字，跟随对应出牌上方居中显示。
- 弃牌和出牌清场统一向右飞出屏幕外，补牌由牌堆飞向手牌区。
- 初始化发牌和补牌使用牌背飞行动画，接近手牌位置后翻成正面，收尾不再闪烁或跳位。

### 设计经验

- 手牌排序是可读性规则，不是抽牌规则。只应改变手牌展示，不应影响牌堆、弃牌堆或 seed。
- 飞牌动画要用克隆牌承载运动，真实手牌只负责最终状态；两者切换时必须冻结过渡，避免明暗闪烁。
- 计分动画应绑定到真正计分的牌，未参与牌型的牌保持安静，玩家才容易理解为什么加了这些筹码。

## 2026-05-30

### 当前状态

- 网页 MVP 已从早期插槽/模块原型转向扑克牌型原型。
- 基础牌使用真正的 52 张牌组，点数为 `2-10 / J / Q / K / A`。
- 已实现抽牌堆和弃牌堆。
- 已实现标准扑克牌型识别。
- 当前 UI 已接入选牌、摊牌、换牌、目标分数、摊牌次数和上头值。
- 底层逻辑已支持赌鬼/Joker 修正、牌型升级和 Boss 负面规则；当前主 UI 已接入商店基础流程，但出售、牌型升级、完整开包和 Boss 展示仍未完成。
- 初始局不再免费携带 Joker 风格修正。
- 选中牌与已提交出牌分离：选中手牌不会立刻复制到出牌区。
- 可以通过 `?seed=` 复现随机序列。
- 过关行为会保留 `上头值`，并按普通关、精英关、Boss 关分别减少 `25`、`35`、`50`，重置已选择的 `红眼赌注`。
- 失败优先级为：`上头值 >= 160` 立即爆牌，优先于胜利；摊牌次数耗尽且未达标时触发 `庄家通吃`。

### UI 变化

- 倍率结算改为按来源 step 顺序播放：来源牌反馈、红倍率滚动、当前分数跟随跳动，并区分加法、乘法和暗涌的演出强度。
- 主界面重新聚焦为五个常驻区域：
  - 左侧信息栏
  - 顶部赌鬼区
  - 中央出牌区
  - 底部手牌区
  - 右侧操作区
- 左侧 HUD 围绕目标分数、当前分数、当前牌型、筹码 × 倍率、赌资/筹码、上头值和剩余操作重新设计。
- 当前视觉方向变为黑暗赌桌、旧纸牌、赌鬼卡、红眼和锁定的红眼赌注。
- 主游玩界面移除了常驻商店、日志和额外面板。
- 主 UI 画布使用固定 `1600 x 900` 等比缩放。
- 早期效果图目标使用纯 DOM/CSS 制作；当前运行时允许使用仓库内本地图片资源，但仍不使用外部字体、前端框架或 Canvas。

### 设计经验

- UI 信息必须服务核心循环：玩家能否过关、还剩几次操作、上头值有多危险、之后有多少经济空间。
- 预览和结算要分离。预览帮助玩家决策；结算应该逐步揭示牌型、单牌得分、修正触发和风险变化。
- 选中的手牌和已提交的出牌不能视觉混在一起。
- 基础牌必须保持简单。复杂度属于赌鬼修正、Boss 规则、商店、卡包和红眼赌注。
- 静态赌鬼卡不应比主动出牌区域更亮。
- 还原参考图时，信息结构和屏幕比例比单个边框装饰更重要。

### 验证习惯

- 语法检查：`node --check web/app.js`。
- 逻辑测试：`node --test web/tests/logic-pure.test.mjs`。
- Diff 空白检查：`git diff --check`。
- 使用固定 seed 做平衡和 UI 回归检查。

### 下一步建议

1. 用多个固定 seed 测试玩家是否会自然追逐高 `上头值`。
2. 强化红眼进入、退出和爆牌阈值的反馈。
3. 增加 DOM 级回归检查，覆盖选择、换牌、摊牌、过关、爆牌失败和红眼赌注。
4. 校准目标曲线、上头值增长、过关减压和赌鬼修正强度。
