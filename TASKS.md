# 任务清单

本文件记录架构整理过程中发现的后续工作。以下项目均为建议，不在本次轻量级整理中执行。

## 已完成的第一阶段整理

- 已将红眼赌注、赌鬼、卡包和赌鬼展示说明集中到 `web/game-content.js`。
- 已将统一运行时状态初始化与重置提取到 `web/runtime-state.js`。
- 已将纯回合判定提取到 `web/round-rules.js`。
- 已将通用动画工具和 DOM 查询分别提取到 `web/animations.js`、`web/game-dom.js`。
- 已增加架构模块测试和固定 seed 浏览器流程测试，覆盖基础出牌/换牌、红眼赌注、通关结算、商店购买、商店进入下一轮、爆牌失败、庄家通吃和失败后重开。
- 已明确文档职责：`PROJECT_CONTEXT.md` 做全局入口和改动导航，`docs/component_rules.md` 做当前规则事实来源，领域 README 说明实现位置。
- 已将上线 MVP / Steam 1000 愿望单目标拆入 `docs/launch_goal.md`。
- 已修正 GitHub Pages 发布 artifact，线上发布会同时包含 `web/`、`art/` 和 `public/`。
- 已增加 `web/tests/deploy-assets.test.mjs`，保护 Pages artifact 目录策略和关键运行时图片路径。
- 已补充 Steam 愿望单转化材料草案，包括商店页文案、截图/视频镜头、胶囊图需求和反馈入口。
- 已产出首批真实 gameplay 截图到 `art/steam/screenshots/`，覆盖主界面、摊牌、红眼赌注、暗涌、通关收益、商店和爆牌失败。

## 最值得优先重构的 3 个点

1. 继续拆分 `web/app.js` 中的商店、红眼生命周期和领域动画编排。
2. 将赌鬼实际规则实现进一步接入集中定义，减少 `game-content.js` 与 `logic-pure.js` 的双处维护。
3. 扩展固定 seed 浏览器测试，覆盖失败、通关、商店与红眼赌注流程。

## P0：优先处理

### 通过 Gate A：公开试玩闸门

**现状**

- 自动测试和浏览器流程测试已有基础覆盖。
- `docs/launch_goal.md` 已明确 Gate A / B / C 发布闸门。
- 已完成一次线上 URL 真实 HTTP 冒烟检查，确认当前 Pages 入口和关键图片资源可访问。
- 已补充桌面与手机横屏布局自动化 QA，检查固定画布无滚动、手牌不压右侧操作区、右侧操作区不越界。
- 已接入 GitHub Issue Form 和独立反馈说明页，用于收集版本信息、玩法理解、红眼使用、重开欲望和愿望单意愿。
- 已新增 `Web MVP Tests` GitHub Actions 工作流，在主干、PR 和手动触发时运行语法检查、Node 测试、浏览器流程测试和空白检查。
- 已复查 `2009dc8` 的 `Web MVP Tests` 与 Pages 部署均成功，线上入口、反馈页、关键资源和 build SHA 均通过 HTTP 冒烟。
- 已修复 Windows headless 浏览器 profile 清理偶发 `EBUSY` 导致测试误失败的问题，本机 `web/tests/browser-flow.test.mjs` 已通过。
- 已在 Pages 部署工作流中接入部署后线上 URL 冒烟检查，覆盖试玩入口、反馈页、关键资源、`web/app.js` 和 build SHA。
- 当前仍缺少真机/人工 QA 记录和规则争议确认。

**建议后续执行**

- 用真机或现成浏览器工具按手动 QA 清单复核手机横屏可读性和截图状态，不新增截图脚本。
- 确认普通摊牌上头公式：继续使用“有效牌点数 + 牌型上头”，或改为“只算有效牌点数”。

### 增加浏览器流程回归测试与上线 QA 证据

**现状**

- 已有固定 seed 浏览器测试覆盖初始化、选牌、换牌、摊牌和重载。
- CI 已接入同一套浏览器流程测试，并在运行前显式定位 Chrome/Chromium。
- Tips、多种红眼赌注、Boss 盲注和更多 seed 分支仍依赖人工检查。

**建议后续重构**

- 扩展现有固定 seed DOM 级测试。
- 优先新增：不同红眼赌注、Boss 盲注流程和更多固定 seed。
- 将 `docs/launch_goal.md` 的 P0 QA 清单沉淀成每次公开试玩前的手动验收记录。

### 保持文档与实现同步

**现状**

- 已建立文档职责和改动导航。
- 规则、流程、UI、资源或测试方式变更时，需要同步更新对应文档。

**建议后续维护**

- 继续以 `logic-pure.js` 测试作为单手规则数值证据，以 `round-rules.js` 测试保护纯回合规则。
- 每次规则改动后检查 `docs/component_rules.md` 和对应领域 README。
- 每次流程或架构改动后检查 `PROJECT_CONTEXT.md` 与 `ARCHITECTURE.md`。

### 产出第一批 Steam 页面素材

**现状**

- `docs/launch_goal.md` 已包含商店页文案草案、截图 shot list、短视频脚本、胶囊图需求和反馈问题。
- 已产出首批 `1600 x 900` 真实 gameplay 截图到 `art/steam/screenshots/`，可用于 Steam 页面初筛和后续裁切。
- 尚未完成最终 Steam 审核包、视频/GIF、胶囊图和 Steam 页面实填素材。
- Steam Coming Soon 闸门要求素材与截图遵守 Steamworks 官方页面要求，商店页送审和公开节奏需要预留时间。

**建议后续执行**

- 从 `art/steam/screenshots/` 中筛选至少 5 张首发截图，必要时用 Browser / Chrome / Edge / Playwright 等现成工具补图，不在仓库里新增自制截图脚本。
- 录制 15-30 秒短视频，优先展示“选牌 -> 上头上涨 -> 红眼赌注 -> 暗涌 -> 结果”。
- 为胶囊图整理可交付给美术的参考图和标题安全区。
- 在 Steam 页面实填前复核官方 Store Assets、Coming Soon、Review Process 和 Wishlists 文档。

## P1：高价值架构改进

### 增加部署后线上 URL 可访问性检查

**现状**

- Pages artifact 已包含 `web/`、`art/` 和 `public/`，并有本地测试保护。
- 已在 `.github/workflows/deploy-pages.yml` 中接入自动线上 URL 冒烟，检查 `/web/`、反馈页、关键图片、`/web/app.js` 和构建 SHA。

**建议后续重构**

- 后续可把冒烟检查结果进一步写入 CI 摘要，方便非技术成员查看。

### 拆分 `web/app.js` 的职责

**现状**

`app.js` 仍同时负责：

- 输入事件。
- 商店。
- 红眼生命周期提交。
- 结算流程。
- 领域动画调度。

**风险**

- 修改局部流程容易影响无关区域。
- 难以为完整回合编写测试。

**建议后续重构**

- 继续提取商店、红眼生命周期和领域动画编排。
- 保留现有行为和数据结构，避免一次性重写。

### 统一玩家术语与内部术语

**现状**

规则核心仍大量使用 `pressure`、`module`、`redline`、`ignitionSequence` 等历史命名。

**风险**

- 新开发者容易误判当前产品概念。
- 文档与代码检索成本高。

**建议后续重构**

- 在稳定测试保护下逐步统一内部命名。
- 不要只做全局替换，应先明确数据契约。

### 为动画建立统一调度边界

**现状**

- 动画依赖多个 `wait`、定时器、class 和动态克隆节点。
- 资源动画与 CSS 动画分别触发。

**风险**

- 动画中断、快速重开或阶段切换时可能残留节点/class。
- 动画时长调整容易破坏流程时序。

**建议后续重构**

- 建立可取消的动画调度器。
- 统一清理临时 DOM、class 和定时器。
- 保留资源加载失败时的降级路径。

## P2：后续维护

### 集中赌鬼定义

**现状**

- 赌鬼商品数据与显示说明已集中，但实际规则仍位于 `logic-pure.js`。

**建议后续重构**

- 保持 `game-content.js` 为规则参数、商品字段和展示字段的单一注册表。
- 后续评估将规则处理器也注册到稳定接口，减少 `logic-pure.js` 中的 ID 分支。

### 明确卡包实现状态

**现状**

- 商店有卡包报价和购买入口。
- 降压道具有实际效果。
- 其他卡包当前主要消耗赌资和展示购买反馈，没有完整开包内容流程。

**建议后续重构**

- 明确卡包是占位演示还是下一阶段功能。
- 在实现前补充规则和测试。

### Python CLI 原型清理

**现状**

- 较早的 `src/jokermvp/` Python CLI 原型已移除。
- 当前规则事实来源、可运行入口和测试边界都集中在 `web/`。

**建议后续维护**

- 不再恢复旧 CLI 原型作为平行规则实现。
- 若后续需要命令行调试工具，应复用 `web/logic-pure.js` 或先明确新的复用边界，避免再次出现双套规则。

### 建立资源清单和压缩流程

**现状**

- `art/` 包含高分辨率 PNG/JPG。
- 当前没有尺寸预算、压缩或预加载策略。

**建议后续重构**

- 记录资源用途、尺寸、体积和触发点。
- 对运行时资源评估 WebP、AVIF 或序列帧方案。
