...
5. 逐张结算有效计分牌。
6. 应用红眼赌注与赌鬼修正。
7. 汇总普通牌、牌型、暗涌牌和赌鬼带来的上头值。
8. 再次应用红眼状态变化。
9. 检查爆牌保险和风险状态。
10. 返回最终分数、上头值和倍率事件。

详细说明见 [结算目录](docs/scoring/README.md)。
...
固定 seed 浏览器测试 `web/tests/browser-flow.test.mjs` 已覆盖初始化、选牌、换牌、摊牌和同 seed 重载一致性。红眼弹窗、通关、商店和失败分支仍缺少浏览器流程覆盖。
